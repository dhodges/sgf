...
func TestGameInfoJsonKeys(t *testing.T) {
	...
	assert.Equal(t, "Boardsize",       keys[0], "")
	assert.Equal(t, "Charset",         keys[1], "")
	assert.Equal(t, "Comment",         keys[2], "")
	assert.Equal(t, "Date",            keys[3], "")
	assert.Equal(t, "Event",           keys[4], "")
	assert.Equal(t, "Komi",            keys[5], "")
	assert.Equal(t, "Place",           keys[6], "")
	assert.Equal(t, "PlayerBlackName", keys[7], "")
	assert.Equal(t, "PlayerBlackRank", keys[8], "")
	assert.Equal(t, "PlayerWhiteName", keys[9], "")
	assert.Equal(t, "PlayerWhiteRank", keys[10], "")
	assert.Equal(t, "Result",          keys[11], "")
}
